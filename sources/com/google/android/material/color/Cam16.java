...
final class Cam16 {
    ...
    static Cam16 fromIntInViewingConditions(int argb, ViewingConditions viewingConditions) {
        ...
        float f10 = atan2;
        float hue2 = atanDegrees < 0.0f ? atanDegrees + 360.0f : atanDegrees >= 360.0f ? atanDegrees - 360.0f : atanDegrees;
        float hueRadians = (hue2 * 3.1415927f) / 180.0f;
        float ac = viewingConditions.getNbb() * p2;
        float f11 = atanDegrees;
        float f12 = ac;
        float f13 = p2;
        float f14 = x;
        float j2 = ((float) Math.pow((double) (ac / viewingConditions.getAw()), (double) (viewingConditions.getC() * viewingConditions.getZ()))) * 100.0f;
        float f15 = y;
        float q2 = (4.0f / viewingConditions.getC()) * ((float) Math.sqrt((double) (j2 / 100.0f))) * (viewingConditions.getAw() + 4.0f) * viewingConditions.getFlRoot();
        ...
        float c = ((float) Math.sqrt(((double) j2) / 100.0d)) * alpha;
        float m2 = viewingConditions.getFlRoot() * c;
        float jstar2 = (1.7f * j2) / ((0.007f * j2) + 1.0f);
        float f22 = alpha;
        float f23 = u2;
        float mstar = ((float) Math.log1p((double) (m2 * 0.0228f))) * 43.85965f;
        float f24 = eHue;
        float f25 = p1;
        float f26 = t;
        return new Cam16(hue2, c, j2, q2, m2, ((float) Math.sqrt((double) ((viewingConditions.getC() * alpha) / (viewingConditions.getAw() + 4.0f)))) * 50.0f, jstar2, ((float) Math.cos((double) hueRadians)) * mstar, ((float) Math.sin((double) hueRadians)) * mstar);
    }
    ...
    private static Cam16 fromJchInViewingConditions(float j2, float c, float h, ViewingConditions viewingConditions) {
        float f = j2;
        float q2 = (4.0f / viewingConditions.getC()) * ((float) Math.sqrt(((double) f) / 100.0d)) * (viewingConditions.getAw() + 4.0f) * viewingConditions.getFlRoot();
        float m2 = c * viewingConditions.getFlRoot();
        float s2 = ((float) Math.sqrt((double) ((viewingConditions.getC() * (c / ((float) Math.sqrt(((double) f) / 100.0d)))) / (viewingConditions.getAw() + 4.0f)))) * 50.0f;
        float hueRadians = (3.1415927f * h) / 180.0f;
        float jstar2 = (1.7f * f) / ((0.007f * f) + 1.0f);
        float mstar = ((float) Math.log1p(((double) m2) * 0.0228d)) * 43.85965f;
        return new Cam16(h, c, j2, q2, m2, s2, jstar2, mstar * ((float) Math.cos((double) hueRadians)), mstar * ((float) Math.sin((double) hueRadians)));
    }
