...
public class FlexBuffersBuilder {
    ...
    private Value createVector(int key, int start, int length, boolean typed, boolean fixed, Value keys) {
        ...
        if (!fixed || typed) {
            ...
            for (int i4 = start; i4 < this.stack.size(); i4++) {
                bitWidth = Math.max(bitWidth, this.stack.get(i4).elemWidth(this.bb.writePosition(), i4 + prefixElems));
                if (!typed) {
                    int i5 = start;
                } else if (i4 == start) {
            ...
            if (!fixed || FlexBuffers.isTypedVectorElementType(vectorType)) {
                ...
                if (!typed) {
                    for (int i8 = start; i8 < this.stack.size(); i8++) {
                        this.bb.put(this.stack.get(i8).storedPackedType(bitWidth));
                    }
                }
                if (value != null) {
                    i = 9;
    ...
    private Value createKeyVector(int start, int length) {
        ...
        for (int i = start; i < this.stack.size(); i++) {
            bitWidth = Math.max(bitWidth, Value.elemWidth(4, 0, (long) this.stack.get(i).key, this.bb.writePosition(), i + 1));
        }
